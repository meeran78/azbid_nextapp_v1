...
const options = {
  ...
  hooks: {
    ...
    //   // After sign-up, update user role if it was provided
    //   if (ctx.path === "/sign-up/email" && ctx.response?.user?.id) {
    //     // Read role from context
    //     const role = (ctx.context as any)?._role || "BUYER";
        
    //     if (role && ["BUYER", "SELLER", "ADMIN"].includes(role)) {
    //       await prisma.user.update({
    //         where: { id: ctx.response.user.id },
    //         data: { role },
    //       });
    //     }
    //   }
    ...
    user: {
      create: {
        before: async (user, context) => {
          ...
          // Default to BUYER, but role will be set in after hook
          let role = "BUYER";
